...
# Кастомное окружение для обучения
class SnekEnv(gym.Env):

    def __init__(self):
        super(SnekEnv, self).__init__()
        # Определяем размеры observation и action (что будет видеть агент и какие у него могут быть действия)
    ...
    def step(self, action):
        ...
        # Меняем направление головы змеи, если нажата соответствующая кнопка
        ...
        # При столкновении завершаем игру
    ...
    def reset(self):
        self.img = np.zeros((500, 500, 3), dtype='uint8')
        # Начальные координаты змеи и яблока
        ...
        for i in range(SNAKE_LEN_GOAL):
            self.prev_actions.append(-1)  # для хранения истории действий

        # Составляем observation:
        observation = [head_x, head_y, apple_delta_x, apple_delta_y, snake_length] + list(self.prev_actions)
        observation = np.array(observation)
